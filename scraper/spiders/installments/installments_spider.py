...
class InstallmentsSpider(Spider):
    ...
    def parse(self, response):
        ...
        return FormRequest.from_response(
            ...
            formdata={
                ...
                utils.stringify(self.account_installment_dict.keys())
            },
    ...
    def after_save_installments_navigation(self, response, page_number=1, modified=set()):
        account_nos = utils.extract_installment_account_nos(response)
        for index, account_no in enumerate(account_nos):
            if (
                no_of_installment :=
                self.account_installment_dict[account_no]
        ...
        if total_accounts > page_number * CONST.ACCOUNTS_PER_PAGE:
            ...
            yield self.pay_saved_installments_request(
                response,
                self.after_pay_saved_installment_navigation
            )

    def after_pay_saved_installment_navigation(self, response):
        yield utils.extract_reference_token_item(response)

    def goto_accounts_list_page_number_request(self, response, page_number, selected_data, callback):
        selected_data[
    ...
    def goto_installments_list_page_number_request(self, response, page_number, callback):
        return FormRequest.from_response(
            response,
            formdata={
                CONST.InstallmentsPage.GOTO_PAGE_NUMBER_INPUT: str(page_number)
            },
    ...
    def update_no_of_installments_request(self, response, index, account_no, no_of_installments, page_number, modified, callback):
        return FormRequest.from_response(
            response,
            formdata={
                CONST.InstallmentsPage.RADIO_BUTTON: str(index),
                CONST.InstallmentsPage.NO_OF_INSTALLMENTS_INPUT:
                str(no_of_installments)
            },
            clickdata={
                'name': CONST.InstallmentsPage.SAVE_NO_OF_INSTALLMENTS_BUTTON
            },
            callback=callback,
            cb_kwargs={
