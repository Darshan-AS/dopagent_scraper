...
from scraper.loaders import AccountLoader


def account_counter_to_page_index(
    account_counter, accounts_per_page=CONST.ACCOUNTS_PER_PAGE
):
    return (
        ceil(account_counter / accounts_per_page),
        (account_counter - 1) % accounts_per_page,
    )


def extract_account_item(response):
    def get_css(_id):
        return f'div p span[id="{_id}"]::text'
